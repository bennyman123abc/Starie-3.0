...
if (user["stats"] != undefined)
{
	...
	// Send the embed.
	message.channel.send("Enter the name of the item you wish to buy. Seperate item names with commas to purchase multiple or type `cancel` to cancel.", { embed : emb });
	
	// Create a message collector
    const collector = message.channel.createMessageCollector(m => m.author.id == message.author.id, { time: 50000 });
	...
	for (var i in items)
	{
		...
		if (list[itemName] != undefined)
		{
		    if (user["money"] >= list[itemName]["price"])
			{
				...
				user["stats"]["equipment"][list[itemName]["type"]] = {"name" : list[itemName]["name"], "might" : list[itemName]["might"]};
...
function generateShopList()
{
	...
	for (var i=0;i<5;i++)
	{
		...
		if (material == "Leather")
		{
			...
			if (mhi <= 0.2)
			{
				var item = {"type" : type, "might" : might, "name" : `${material} ${type}`, "price" : price};
			}
			else
			{
				var item = {"type" : type, "might" : might, "name" : `${material}`, "price" : price};
			}
