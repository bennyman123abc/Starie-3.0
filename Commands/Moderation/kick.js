...
if (args.length == 2)
{
	...
	{
	    // Prevent the kicking of yourself.
        if (getUserResolvable(args[0], message.guild.id).id != message.author.id)
        {
            // Check if the user can be kicked.
			if (message.guild.members.get(getUserResolvable(args[0], message.guild.id).id).kickable)
			{
			    // Post the embed.
				...
				emb.addField("Action", "Kick");
				emb.addField("Reason", args[1]);
				emb.setColor("#ffaa00");
				message.channel.send({ embed : emb });
				message.channel.send("Would you like to kick this user? __y__es or __n__o?");
				...
				if (m.content == "y")
				{
				message.guild.members.get(getUserResolvable(args[0], message.guild.id).id).kick();
			...
			{
			    // Error if the user cannot be kicked.
			    message.reply(`I cannot kick user \`${getUserResolvable(args[0], message.guild.id).tag}\``);
			}
        }
       	else
		{
